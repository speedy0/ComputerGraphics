...
public:
	...
	void TRender(string filename1, string filename2);

	void CreateTrack();
	void RenderTrack();
	void CreateBarrierTrack();
	void RenderBarrierTrack();
...
private:
	...
	GLuint m_vaoTrack;
	GLuint m_vaoRightTrack;
	GLuint m_vaoBarrierTrack;
	...
	vector<glm::vec3> m_leftBarrierPoints; 
	vector<glm::vec3> m_rightBarrierPoints;


	unsigned int m_vertexCount;				// Number of vertices in the track VBO
	unsigned int m_barrierVertexCount;
};
